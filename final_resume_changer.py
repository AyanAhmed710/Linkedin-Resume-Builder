...
OUTPUT_PDF = r"D:\LANGCHAIN\langchain_Resume_Builder\resume_filled_final.pdf"
...
def build_title_overlay():
    ...
    # White-out old title — starts at x=9 to cover "Software" which begins at x=11.91
    c.setFillColorRGB(1, 1, 1)
    c.rect(9, y_bottom - 2, 480, height + 5, fill=1, stroke=0)

    # Write new title at same x position as original text (x0=11.91)
    c.setFillColorRGB(0.40, 0.40, 0.40)
    c.setFont("Helvetica", 12)
    c.drawString(11.91, y_bottom + 1, JOB_TITLE)
...
def build_sections_overlay():
    ...
    # White-out the existing "Courses :" and "Skills :" headings
    # that are already baked into the original PDF (coordinates from pdfplumber)
    c.setFillColorRGB(1, 1, 1)
    # Courses heading: top=574.96, bottom=589.31
    c.rect(80, to_rl(589.31) - 2, 200, (to_rl(574.96) - to_rl(589.31)) + 4, fill=1, stroke=0)
    # Skills heading: top=711.52, bottom=725.86
    c.rect(80, to_rl(725.86) - 2, 200, (to_rl(711.52) - to_rl(725.86)) + 4, fill=1, stroke=0)

    y = to_rl(PROJECTS_CONTENT_TOP) - GAP_BELOW_HEADING - 6
